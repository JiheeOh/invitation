...
export default function Family({ t, onContact }: FamilyProps) {
    ...
    return (
        ...
        >
            ...
            >
                    {People.map((p, i) => (
                        <div key={i} data-testid="family-portrait-container" onContextMenu={(e) => e.preventDefault()} onCopy={(e) => e.preventDefault()} style={{ position: 'relative', aspectRatio: '1/1', overflow: 'hidden', userSelect: 'none' }}>
                            {p.photo ? (
                                <>
                                    <Image
                                        src={p.photo}
