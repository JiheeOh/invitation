...
export function InterviewButton({ t, onOpen }: InterviewButtonProps) {
    return (
        ...
        >
            <FadeIn>
                <SectionLabel t={t} eng="wedding interview" ko="웨딩 인터뷰" />
                <div
                    style={{
                        fontFamily: t.serif,
                        fontSize: 13,
...
export function InterviewModalContent({ t }: { t: Theme }) {
    ...
    const Speaker = ({ emoji, name, text, color }: { emoji: string; name: string; text: string; color: string }) =>
        text ? (
        ...
        <div>
            {Q.map((it, i) => (
                ...
                >
                    ...
                    <Speaker emoji="🤵" name={WEDDING.groom.name} text={it.groom} color="#7AA0CC" />
                    <Speaker emoji="👰" name={WEDDING.bride.name} text={it.bride} color="#D08A8A" />
                </div>
            ))}
        </div>
    );
